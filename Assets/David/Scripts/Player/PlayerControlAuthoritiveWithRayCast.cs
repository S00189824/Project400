...
public class PlayerControlAuthoritiveWithRayCast : NetworkBehaviour
{
    [SerializeField]
    ...
    [SerializeField]
    private NetworkVariable<float> networkPlayerAttackBlend = new NetworkVariable<float>();
    ...
    [SerializeField]
    private float minAttackDistance = 1.0f;
    ...
    private void FixedUpdate()
    {
        if (IsClient && IsOwner)
        {
            if (networkPlayerState.Value == PlayerState.Attack && ActiveAttackActionKey())
            {
                CheckHit(leftHand.transform, Vector3.up);
                CheckHit(rightHand.transform, Vector3.down);
            }
        }
    }

    private void CheckHit(Transform hand, Vector3 aimDirection)
    {
        RaycastHit hit;

        int layerMask = LayerMask.GetMask("Player");

        if (Physics.Raycast(hand.position, hand.transform.TransformDirection(aimDirection), out hit, minAttackDistance, layerMask))
        {
            Debug.DrawRay(hand.position, hand.transform.TransformDirection(aimDirection) * minAttackDistance, Color.yellow);
        ...
        {
            Debug.DrawRay(hand.position, hand.transform.TransformDirection(aimDirection) * minAttackDistance, Color.red);
        }
    }
    ...
    private void ClientVisuals()
    {
        if (oldPlayerState != networkPlayerState.Value)
        {
            ...
            if (networkPlayerState.Value == PlayerState.Attack)
            {
                animator.SetFloat($"{networkPlayerState.Value}AttackBlend", networkPlayerAttackBlend.Value);
            }
        }
    }
    ...
    private void ClientInput()
    {
        ...
        if (ActiveAttackActionKey() && forwardInput == 0)
        {
    ...
    private static bool ActiveAttackActionKey()
    {
    ...
    public void UpdateHealthServerRpc(int takeAwayPoint, ulong clientId)
    {
        var clientWithDamaged = NetworkManager.Singleton.ConnectedClients[clientId]
            .PlayerObject.GetComponent<PlayerControlAuthoritiveWithRayCast>();
    ...
    public void NotifyHealthChangedClientRpc(int takeAwayPoint, ClientRpcParams clientRpcParams = default)
    {
        ...
        Logger.Instance.LogInfo($"Client got Hit {takeAwayPoint}");
    }
    ...
    public void UpdatePlayerStateServerRpc(PlayerState state)
    {
        ...
        {
            networkPlayerAttackBlend.Value = Random.Range(0.0f, 1.0f);
        }
    }
}
